...
def generate_directories_fragment(
    base_src_folder: Path = typer.Argument(..., help="The source folder in which all binaries are located"), 
    target_directory: Path = typer.Argument(Path.cwd(), help="The target directory to which the `Directory.wxs` is written")
):
    """
...
def generate_component_fragment(
    base_src_folder: Path = typer.Argument(..., help="The source folder in which all binaries are located"), 
    component: Component = typer.Argument(..., help="The component type for which to create the fragment."), 
    target_directory: Path = typer.Argument(Path.cwd(), help="The target directory to which the `Directory.wxs` is written")
):
    """
